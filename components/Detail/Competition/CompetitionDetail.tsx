...
function CompetitionDetail() {
  ...
  const [competition, setCompetition] = useState<
    (Competition.Competition & { _id?: string }) | null
  >(null);
  ...
  const formattedCompetition = useMemo(() => {
    return competition
      ? {
          ...
          info: {
            ...competition.info,
            signUpStart: competition.info.signUpStart
              ? moment(competition.info.signUpStart)
              : undefined,
            signUpEnd: competition.info.signUpEnd
              ? moment(competition.info.signUpEnd)
              : undefined,
            time: competition.info.time
              ? moment(competition.info.time)
              : undefined,
  ...
  const edit = useCallback(async () => {
    setLoading(true);
    if (competition) {
      competition.updatedTime = moment(new Date()).format(
        "YYYY-MM-DD HH:mm:ss"
      );
  ...
  const add = useCallback(async () => {
    ...
    if (competition && loginStatus) {
      ...
      competition.createdTime = moment(new Date()).format(
        "YYYY-MM-DD HH:mm:ss"
      );
      competition.updatedTime = moment(new Date()).format(
        "YYYY-MM-DD HH:mm:ss"
        ...
        <Layout.Content>
          <CompetitionContext.Provider value={{ competition, setCompetition }}>
            <div className={styles["container"]}>
              ...
              <div className={styles["part-container"]}>
                ...
                {competition && (
                  <Form
                    labelCol={{ span: 9 }}
                    wrapperCol={{ span: 15 }}
                    initialValues={formattedCompetition}
                    form={form}
                  >
              ...
              {competition?.winners && (
                <AwardTable winners={competition?.winners || []} />
              )}
              <div
